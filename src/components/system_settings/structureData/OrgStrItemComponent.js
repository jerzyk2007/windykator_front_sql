...
//       // const result = true;
//       if (result) {
//         setNewDataItem((prevItems) =>
//           prevItems.filter((item) => item.id !== id)
//         );
//         setStartData((prevItems) =>
//           prevItems.filter(
//             (item) => item[`id_${info.toLowerCase()}_items`] !== id
//           )
//         );
//       }
...
//         ? startData.some(
//             (item) =>
//               (item[`id_${info.toLowerCase()}_items`] !== id &&
...
//       (item) => item.oldMail === newValue.toLowerCase()
//     );
...
//           if (
//             item[`id_${info.toLowerCase()}_items`] === updateData.id &&
//             info !== "OWNER"
//           ) {
//             return {
//               ...item,
...
//                 <input
//                   style={
//                     mailDuplicate ? { color: "red", fontWeight: "bold" } : null
//                   }
//                   className="item_component-title__container-data--text"
//                   type="text"
...
//                 <input
//                   style={
//                     duplicate ? { color: "red", fontWeight: "bold" } : null
//                   }
//                   className="item_component-title__container-data--text"
//                   type="text"
//                   placeholder={
...
//                         addItem.newMail
//                           ? true
//                           : false && addItem.company
//                           ? true
//                           : false && addItem.newMail
//                           ? true
//                           : false)
...
const OrgStrItemComponent = ({ data, info, title, multiCompany }) => {
  ...
  const handleActiveItem = (index) => {
    if (!editIndex) {
      setDuplicate(false); // Resetujemy błędy przy wejściu w edycję
  ...
  const handleEdit = (e, id, type) => {
    ...
    // Znajdź element po aktualizacji (z tablicy update)
    const currentItem = update.find((item) => item.id === id);
    const nameFilter = newFilterData.filter((item) => item.id === id); // stare dane do referencji

    // Ustalanie wartości do sprawdzenia (bierzemy z inputa lub ze starej wartości jeśli nie zmieniano tego pola)
    const name = type === "name" ? newValue : nameFilter[0].oldName; // tu można brać z currentItem.newName dla spójności
    const company = type === "company" ? newValue : nameFilter[0].company;

    // --- Walidacja duplikatu NAZWY (Oryginalna logika) ---
    const checkDuplicate =
      info !== "AREA"
        ? startData.some(
            (item) =>
    ...
    // --- ZMIANA: Walidacja duplikatu MAILA przy zmianie FIRMY (lub nazwy, jeśli to wpływa na logikę) ---
    // Sprawdzamy tylko jeśli edytujemy OWNERA
    if (info === "OWNER") {
      // Pobieramy aktualny mail (nowy jeśli edytowany, stary jeśli nie)
      const currentMail = currentItem.newMail
        ? currentItem.newMail
        : currentItem.oldMail;

      if (currentMail) {
        // Sprawdzamy czy w NOWEJ firmie istnieje już taki mail (pomijając edytowany element)
        const checkMailDuplicate = update.some(
          (item) =>
            item.id !== id && // nie ten sam element
            item.oldMail?.toLowerCase() === currentMail.toLowerCase() &&
            item.company === company // w tej samej firmie
        );
  ...
  const handleEditMail = (e, id) => {
    ...
    // Znajdujemy aktualną firmę edytowanego elementu
    const currentItem = updateNewDataIltem.find((item) => item.id === id);
    const currentCompany = currentItem ? currentItem.company : "";

    // --- ZMIANA: Sprawdzamy duplikat maila ORAZ firmy ---
    const checkDuplicate = newDataItem.some(
      (item) =>
        item.id !== id && // nie porównuj z samym sobą
        item.oldMail?.toLowerCase() === newValue.toLowerCase() &&
        item.company === currentCompany // musi zgadzać się też firma
    );
    setMailDuplicate(checkDuplicate);
        ...
        const update = newDataItem.map((item) => {
          if (item.id === updateData.id) {
            return {
              ...
              oldName: item.newName,
              oldCompany: item.company,
              oldMail: item.newMail, // Aktualizujemy też oldMail po zapisie
            };
          } else {
            return item;
          }
  ...
  const handleAddItem = (e, type) => {
    ...
    // Obliczenie nowych wartości dla stanu przed setAddItem (żeby mieć dostęp synchronicznie)
    ...
    // --- ZMIANA: Walidacja duplikatu MAILA przy zmianie FIRMY (dla dodawania) ---
    if (info === "OWNER") {
      // Maila bierzemy z obecnego stanu (nie zmienia się w tej funkcji)
      const currentMail = addItem.newMail;

      if (currentMail) {
        const checkMailDup = newDataItem.some(
          (item) =>
            item.oldMail?.toLowerCase() === currentMail.toLowerCase() &&
            item.company === company // Sprawdzamy w nowo wybranej firmie
        );
        setMailDuplicate(checkMailDup);
      }
    }
  };
  ...
  const handleAddMail = (e) => {
    ...
    const checkDuplicate = newDataItem.some(
      (item) =>
        item.oldMail?.toLowerCase() === newValue.toLowerCase() &&
        item.company === addItem.company // sprawdzamy w aktualnie wybranej firmie
    );
  ...
  const handleAcceptNewItem = async () => {
    try {
      ...
      if (addItem) {
        ...
        // const result = true;
        if (result) {
          // Tutaj warto upewnić się, że nowy element ma strukturę zgodną z listą
          const update = [
            ...newDataItem,
            { ...addItem, id: result.data.newId || Date.now() },
          ]; // Dodałem fake ID w razie braku z backendu
          setNewDataItem(update);
          // Ponowne pobranie danych z backendu jest zwykle bezpieczniejsze,
          // ale tutaj aktualizujemy startData na podstawie odpowiedzi:
          if (Array.isArray(result.data)) {
            setStartData(result.data);
          } else {
            // Fallback jeśli backend zwraca tylko id, albo inny obiekt
            // To zależy od Twojego API, zostawiam tak jak było w oryginale lub dostosowuje
            // W oryginale: setStartData(result.data); Zakładam że zwraca pełną listę.
            setStartData(result.data);
          }
        }
      }
