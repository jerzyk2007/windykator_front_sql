...
const UserTableColumns = ({ user, columns }) => {
  ...
  const handleChangeChecked = (info) => {
    setUserColumns(
      userColumns.map((col) => ({ ...col, checked: info === "all" }))
    );
  };

  const handleSaveUserColumns = async () => {
    const modifiedColumns = userColumns
      .map((col) => {
        if (col.checked) {
    ...
    try {
      await axiosPrivateIntercept.patch(
        `/user/change-columns/${user.id_user}`,
        {
  ...
  return (
    <section className="user-table-columns">
      ...
      <section className="user-table-columns__container">{columnsItem}</section>
      <Button variant="contained" onClick={handleSaveUserColumns} size="small">
        Zmień
      </Button>
    </section>
  );
