...
const EditContractor = ({ contractor, onBack }) => {
  const axiosPrivateIntercept = useAxiosPrivateIntercept();
  const { auth } = useData();
  // --- LOGIKA UPRAWNIEŃ ---
  // Sprawdzamy czy w tablicy roles znajduje się 150 lub 2000
  const isAdmin = auth?.roles?.some((role) => [150, 2000].includes(role));
  const userProfile = isAdmin ? "admin" : "user";

  const [emails, setEmails] = useState(contractor.EMAIL || []);
  const [phones, setPhones] = useState(contractor.TELEFON || []);
  const [pleaseWait, setPleaseWait] = useState(false);
  const [saveError, setSaveError] = useState(null);

  const [statusWindykacji, setStatusWindykacji] = useState(
    contractor.STATUS_WINDYKACJI || 0,
  );
  ...
  const handleSaveData = async (email, phone, status) => {
    ...
    try {
      ...
      // Opcjonalnie powrót po sukcesie: onBack();
    } catch (error) {
      console.error(error);
              ...
              <div className="sm-edit-card ak-bg">
                ...
                <div className="sm-view-field">
                  ...
                  <span>
                    ...
                    {contractor.AK_NRLOKALU ? `/${contractor.AK_NRLOKALU}` : ""}
                  </span>
                </div>
                <div className="sm-view-field">
                  <label>Kod i miejscowość</label>
                  <span>
                    {contractor.AK_KOD || "---"} {contractor.AK_MIASTO || "---"}
                  </span>
                </div>
              </div>
            </section>
                  ...
                  {newEmail.active && (
                    <div className="sm-contact-item sm-new-item-row">
                      <TextField
                        ...
                        value={newEmail.value}
                        error={
                          newEmail.value !== "" && !isEmailValid(newEmail.value)
                        }
                        onChange={(e) =>
                  ...
                  {newPhone.active && (
                    <div className="sm-contact-item sm-new-item-row">
                      <TextField
                        ...
                        value={newPhone.value}
                        error={
                          newPhone.value !== "" && !isPhoneValid(newPhone.value)
                        }
                        onChange={(e) =>
