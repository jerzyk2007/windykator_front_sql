...
const EditRowTable = ({
  ...
  useEffect(() => {
    ...
    // Logika dla Kontroli BL
    else if (
      info === "control-bl" &&
      auth.roles.includes(120) // Sprawdzenie uprawnień dla BL (zgodnie z logiką w Select)
    ) {
      setChangePanel("control-bl");
    }
    // Przyszłościowa logika dla innych opcji
    else if (info === "law-partner" && auth.roles.includes(150)) {
      setChangePanel("law-partner");
    }
  }, [info, rowData?.MARK_FK, auth.roles, rowData.AREA]);
