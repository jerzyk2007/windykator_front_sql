    ...
    newData.forEach((sheet) => {
      ...
      if (sheet.data && sheet.data.length > 0) {
        ...
        headers.forEach((header, columnIndex) => {
          ...
          if (header === 'Dział') {
          ...
          else if (header === 'Stan należności w % - BL' || header === 'Stan wszystkich należności w %' || header === 'Stan  należności bez kancelarii w %' || header.includes('Cele na') || header === 'Stan należności w %') {
            column.numFmt = '0%';
            ...
            for (let rowIndex = excelStartRow; rowIndex <= excelEndRow; rowIndex++) {
              ...
              if (typeof rawValue === 'string' && rawValue.includes('%')) {
                const number = parseFloat(rawValue.replace('%', '').trim().replace(',', '.'));
                if (!isNaN(number)) {
          ...
          else if (header === 'Kwota przeterminowanych fv - BL' || header === 'Kwota nieprzeterminowanych FV - BL' || header === 'Kwota wszystkich przeterminowanych fv' || header === 'Kwota wszystkich nieprzeterminowanych fv' || header === 'Kwota przeterminowanych fv bez kancelarii' || header === 'Kwota wszystkich nieprzeterminowanych fv bez kancelarii' || header === 'Kwota nierozliczonych fv - błędy Doradcy i dokumentacji' || header === "Kwota niepobranych VAT'ów" || header === 'Kwota przeterminowanych FV' || header === 'Kwota nieprzeterminowanych FV') {
            ;
            column.numFmt = '#,##0.00';
          }
          else if (header === 'Ilość przeterminowanych FV - BL' || header === 'Ilość wszystkich faktur przeterminowanych' || header === 'Ilość faktur przeterminowanych bez kancelarii' || header === "Ilość niepobranych VAT'ów" || header === 'Ilość nierozliczonych fv - błędy Doradcy i dokumentacji' || header === 'Ilość przeterminowanych FV' || header === "Kwota niepobranych VAT'ów") {
            column.numFmt = '#,##0';
