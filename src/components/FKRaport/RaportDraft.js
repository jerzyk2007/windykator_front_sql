...
import {
  ...
  Box,
  Chip,
  // Dodane komponenty do okna potwierdzenia
  Dialog,
  DialogTitle,
  DialogContent,
  DialogContentText,
...
const RaportDraft = ({ company }) => {
  ...
  useEffect(() => {
    let timer;
    if (openConfirm && countdown > 0) {
      timer = setInterval(() => {
        setCountdown((prev) => prev - 1);
      }, 1000);
    }
    return () => clearInterval(timer);
  }, [openConfirm, countdown]);

  const handleOpenConfirm = () => {
    setCountdown(5); // Czas blokady w sekundach
    setOpenConfirm(true);
  };
  ...
  const handleCloseConfirm = () => {
    if (countdown === 0) {
      ...
      // Pozwalamy anulować w dowolnym momencie
      setOpenConfirm(false);
    }
  };
    ...
    <main className="raport-draft-page">
      ...
      >
        ...
        >
          ...
          UWAGA: Generowanie nowego raportu
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            Ta operacja zmieni dane Raportu Draft i zamknie bieżący okres.{" "}
            <br />
        ...
        <DialogActions sx={{ p: 3 }}>
          ...
          >
            {countdown > 0 ? `Czekaj (${countdown}s)` : "Tak, przygotuj raport"}
          </Button>
        </DialogActions>
      </Dialog>
              ...
              {!missongDeps ? (
                <div className="action-container">
                  ...
                  <Button
                    ...
                    size="large"
                    startIcon={<SummarizeOutlinedIcon />}
                    onClick={handleOpenConfirm} // Zmienione z createNewRaport na handleOpenConfirm
                  >
                    Przygotuj nowy raport
                  </Button>
                </div>
