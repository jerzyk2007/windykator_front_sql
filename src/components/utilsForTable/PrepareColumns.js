...
export const prepareColumns = (columnsData, data) => {
  const changeColumn = columnsData.map((item) => {
    ...
    if (item.accessorKey === "DATA_FV") {
      modifiedItem.accessorFn = (originalRow) => new Date(originalRow.DATA_FV);
      ...
      //     return date.toLocaleDateString("pl-PL", { useGrouping: true });
      //   } else {
      //     // Jeśli data jest nieprawidłowa, zwracamy pusty string lub inny komunikat błędu
      //     return "brak danych";
      //   }
      // };
      modifiedItem.Cell = ({ cell }) => cell.getValue().toLocaleDateString();
    }
