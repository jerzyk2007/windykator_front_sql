...
const Contacts = () => {
  ...
  const contactsItem = contactsData.map((contact, index) => {
    return (
      <section className="contacts__container" key={index}>
        ...
        <LiaEditSolid
          className="contacts__container--edit"
          onClick={() => setContactItemData(contact)}
        />
      </section>
    );
  });
  ...
  return pleaseWait ? (
    <PleaseWait />
  ) : conctactItemData.name ? (
    <ContactItem
      conctactItemData={conctactItemData}
      setContactItemData={setContactItemData}
      setContactsData={setContactsData}
      contactsData={contactsData}
    />
  ) : (
    <section className="contacts">
      <form className="contacts-search" onSubmit={searchResult}>
        <input
