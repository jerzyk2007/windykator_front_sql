...
const AgingAccount = ({ showTable, filteredDataRaport, setTableData, setShowTable, style }) => {
    ...
    useEffect(() => {
        const agingArray = [...new Set(filteredDataRaport.map(age => age['Przedział']))];
        const ageArray = ['< 1', ' 1 - 30', ' 31 - 90', ' 91 - 180', ' 181 - 360', '> 360'];
    ...
    return (
        <>
            <section className='aging_account' >
                <label
                    className={style === "car" ? 'aging_account--select aging_account--select--car' : "aging_account--select "}
                    onClick={() => setArrow({
                        aging: !arrow.aging
                    })}
                >
                    ...
                    />
                    Wiekowanie</label >
                <label className='aging_account--doc-counter' onDoubleClick={handleClick}>
                    {counter}
                </label>
                <label className='aging_account--doc-sum' onDoubleClick={handleClick}>{sum.toLocaleString('pl-PL', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                    useGrouping: true,
                })}</label>
                <label className='aging_account--percent' onDoubleClick={handleClick}>{percent}</label>
            </section >
