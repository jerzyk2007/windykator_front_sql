...
const AgingAreas = ({ showTable, area, filteredData, setTableData, setShowTable, style }) => {
    ...
    return (
        <>
            <section className="aging_areas"
                style={counter === 0 || showTable ? { display: "none" } : null}
            >
                <label
                    className={style === "car" ? 'aging_areas--select aging_areas--select--car' : "aging_areas--select"}
                    onClick={() => setArrow(
                        {
                            [area]: !arrow[area]
                        }
                ...
                >
                    ...
                    />
                    {area}</label>
                <label className="aging_areas--doc-counter" onDoubleClick={handleClick}>{counter}</label>
                <label className="aging_areas--doc-sum" onDoubleClick={handleClick}>{(sum).toLocaleString('pl-PL', {
